...
export class LeaderboardModal {
  ...
  public async show(onClose: () => void): Promise<void> {
    ...
    this.container.innerHTML = `
      <div class="w-full max-w-3xl bg-gradient-to-b from-slate-900 via-slate-950 to-black border-2 border-amber-500/70 rounded-2xl sm:rounded-3xl p-3 sm:p-6 shadow-[0_0_50px_rgba(0,0,0,0.9)] flex flex-col my-auto max-h-[94vh] overflow-hidden">
        <div class="flex items-center justify-between border-b border-amber-500/30 pb-4 mb-4">
          <div>
            <span class="text-xs font-mono font-bold text-amber-400 uppercase tracking-[0.25em]">${t('global_rankings')}</span>
            <h2 class="text-2xl sm:text-3xl font-gothic font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-100 via-amber-300 to-yellow-500 mt-0.5">${t('leaderboard')}</h2>
          </div>
          <button id="leaderboard-close-btn" class="bg-slate-900 hover:bg-slate-800 border border-slate-700 hover:border-amber-500/50 px-4 py-2 rounded-xl text-xs font-mono font-bold text-slate-200 transition active:scale-95 shadow">
            ${t('close')}
          </button>
        </div>
